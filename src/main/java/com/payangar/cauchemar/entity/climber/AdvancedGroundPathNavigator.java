...
/**
 * A {@link GroundPathNavigation} that drives the surface-aware path finder ({@link
 * AdvancedPathFinder} over {@link AdvancedWalkNodeProcessor}) and adds obstruction-based stuck
 * detection: when blocked along the path it tells the entity via {@code onPathingObstructed} so the
 * AI can react. Base class for the climbing navigators. Ported from Nyf's Spiders.
 */
