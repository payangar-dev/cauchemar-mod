...
/**
 * A {@link PathFinder} variant with a pluggable heuristic and a configurable expansion budget, used
 * as the base for the surface-aware {@link AdvancedPathFinder}. Mirrors vanilla's A* loop but keeps
 * its own {@link BinaryHeap}/options buffer so the climbing path search can be tuned independently.
