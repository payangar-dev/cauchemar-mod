...
/**
 * Move control that steers the spider in surface-local space (so "forward" follows the climbed
 * surface), and triggers a directional jump to round corners onto perpendicular faces. Ported from
 * Nyf's Spiders. {@code isWalkableAtOffset} is stubbed until the climbing node evaluator (step 5).
 */
public class ClimberMoveController<T extends Mob & IClimberEntity> extends MoveControl {
    /** Max yaw turn toward the path target per tick (degrees). Lower = smoother turns, fewer spins;
     *  Nyf's original 90 turned a full circle in 4 ticks, which read as a "spin" while pathing. */
    private static final float MAX_YAW_TURN_PER_TICK = 30.0f;

    protected final IClimberEntity climber;

    @Nullable
    protected net.minecraft.core.BlockPos block;
    ...
    public void setMoveTo(double x, double y, double z, net.minecraft.core.BlockPos block, Direction side, double speedIn) {
    ...
    public void tick() {
        ...
        if (this.operation == Operation.STRAFE) {
            ...
            if (this.side != null && this.block != null) {
                ...
                switch (this.side) {
                    case DOWN:
                        if (aabb.minY >= this.block.getY() + shape.max(Direction.Axis.Y) - 0.01D) ox -= 0.1D;
                        break;
                    case UP:
                        if (aabb.maxY <= this.block.getY() + shape.min(Direction.Axis.Y) + 0.01D) oy += 0.1D;
                        break;
                    case WEST:
                        if (aabb.minX >= this.block.getX() + shape.max(Direction.Axis.X) - 0.01D) ox -= 0.1D;
                        break;
                    case EAST:
                        if (aabb.maxX <= this.block.getX() + shape.min(Direction.Axis.X) + 0.01D) ox += 0.1D;
                        break;
                    case NORTH:
                        if (aabb.minZ >= this.block.getZ() + shape.max(Direction.Axis.Z) - 0.01D) oz -= 0.1D;
                        break;
                    case SOUTH:
                        if (aabb.maxZ <= this.block.getZ() + shape.min(Direction.Axis.Z) + 0.01D) oz += 0.1D;
                        break;
                }
                ...
                if (aabb.intersects(blockAabb)) {
                    ...
                    switch (this.side) {
                        case DOWN:
                            if (aabb.minY + allowedOffset < this.block.getY() + shape.max(Direction.Axis.Y) - 0.01D) oy = 0;
                            break;
                        case UP:
                            if (aabb.maxY + allowedOffset > this.block.getY() + shape.min(Direction.Axis.Y) + 0.01D) oy = 0;
                            break;
                        case WEST:
                            if (aabb.minX + allowedOffset < this.block.getX() + shape.max(Direction.Axis.X) - 0.01D) ox = 0;
                            break;
                        case EAST:
                            if (aabb.maxX + allowedOffset > this.block.getX() + shape.min(Direction.Axis.X) + 0.01D) ox = 0;
                            break;
                        case NORTH:
                            if (aabb.minZ + allowedOffset < this.block.getZ() + shape.max(Direction.Axis.Z) - 0.01D) oz = 0;
                            break;
                        case SOUTH:
                            if (aabb.maxZ + allowedOffset > this.block.getZ() + shape.min(Direction.Axis.Z) + 0.01D) oz = 0;
                            break;
                    }
                }
            ...
            if (targetDist < 0.0001D) {
                ...
                this.mob.setYRot(this.rotlerp(this.mob.getYRot(), 270.0f - (float) Math.toDegrees(Mth.atan2(rx, ry)), MAX_YAW_TURN_PER_TICK));
