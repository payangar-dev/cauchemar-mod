...
/**
 * A path {@link Node} that also carries which surface sides are walkable at this position (packed by
 * {@link AdvancedWalkNodeProcessor}), the side the owning path assigned to it, and whether it is a
 * drop. This is what lets the spider's path describe walls/ceilings, not just floor. Ported from
 * Nyf's Spiders.
 */
public class DirectionalPathPoint extends Node {
    ...
    /** All pathable sides of this node, i.e. all sides the entity could potentially walk on. */
    public Direction[] getPathableSides() {
        return this.pathableSides;
    }

    /** The side assigned to this node by the path it is part of, or null if not yet assigned. */
    @Nullable
    public Direction getPathSide() {
        return this.pathSide;
    }

    /** Whether this node represents a drop. */
